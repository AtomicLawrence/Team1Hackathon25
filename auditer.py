...
def auditer_gpt(input_text: str, input_image: str, filters: Filters, gpt_model: str):
    client = OpenAI(api_key=OAI_API_KEY)

    screen_reader_prompt = """
    support for screen readers (including alt-text, hiding decorative images, heading annotations), 
""" if filters.screen_reader_support else ''
    
    voice_input_prompt = """
    alternative button titles for voice-based input methods, 
""" if filters.voice_input else ''

    colour_contrast_prompt = """
    color alternatives for colorblind users, sufficient contrast, 
""" if filters.sufficient_contrast else ''
...
23. **Foo is not annotated with bar**
  ...
  - **Suggested Change:** line 123
    ...
    These accessibility needs include {screen_reader_prompt}{voice_input_prompt}dark mode support, support for browser zoom & scaling, {colour_contrast_prompt}reduced motion preference, captions, and audio descriptions.
    """,
        input=[
            {
                "role": "user",
...
def chat(user_input: AccessiblityImprovementRequest, filters: Filters) -> str:
    base64_string = encode_image(user_input.screenshot)

    response = auditer_gpt(user_input.text, base64_string, filters, GPT_MODEL)
